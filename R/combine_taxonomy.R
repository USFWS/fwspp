...
#' mb <- fws_occ(fws = mb)
#' mlb <- combine_occ(ml, mb)
#' }

combine_occ <- function(...) {
  occ_list <- list(...)

  # Validating that objects can be combined
  if (length(occ_list) < 2) stop("Need at least two `fwspp` objects to use this function.")
  if (!all(sapply(occ_list, inherits, "fwspp")))
    stop("All inputs must be `fwspp` objects.")
  exp_attr <- c("names", "class", "boundary", "scrubbing", "buffer_km", "query_dt")
  if (!all(sapply(occ_list, function(x) identical(names(attributes(x)), exp_attr))))
    stop("One or more `fwspp` objects is missing expected attributes.")
  if (length(unique(sapply(occ_list, has_taxonomy))) > 1)
  ...
  core_attr <- lapply(occ_list, function(x) attributes(x)[core_attr])
  if (length(unique(core_attr)) != 1)
    stop("Cannot combine `fwspp` objects with different `boundary`, `scrubbing`, ",
         "or `buffer` settings.")

  # Combining
  orgnames <- unlist(lapply(occ_list, function(x) attributes(x)[["names"]]))
  query_dts <- lapply(occ_list, function(x) attributes(x)[["query_dt"]])
  earliest_query <- query_dts[[which.min(query_dts)]]
  out_occ <- do.call("c", occ_list)
  attributes(out_occ) <- c(list(names = orgnames),
                           core_attr[[1]],
