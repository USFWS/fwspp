...
This packages contains functions to perform geographic query of several biodiversity databases based on U.S. Fish and Wildlife Service (USFWS) administrative or acquisition property boundaries (plus optional buffers).  At present, the package queries the [Global Biodiversity Information Facility (GBIF)](http://www.gbif.org/), [Biodiversity Information Serving Our Nation (BISON)](https://bison.usgs.gov/#home), [Integrated Digitized Biocollections (iDigBio)](https://www.idigbio.org/), and [VertNet](http://vertnet.org/).  It draws heavily from the outstanding work by the [ROpenScience group](https://ropensci.org/) and their suite of species occurrence packages.  
...
or later, when you'll be prompted by the `fws_occ` function. The process takes 10 minutes or so to complete.
...
1. Decide on the boundary, scrubbing, and output parameters of that query
1. Run the query

    Okay, four steps:

1. Wait... probably a long while...
